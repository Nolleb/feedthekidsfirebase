...
@Component({
  ...
  imports: [Field, SelectComponent, RouterLink, SvgIconDirective],
...
export class AdminRecipeEditComponent implements OnInit {
  ...
  constructor() {
    ...
    this.loadRecipeSub = entities$
      .pipe(filter((entities) => entities && entities.length > 0))
      .subscribe((entities) => {
  ...
  updateIngredientName(index: number, event: Event) {
    ...
    this.recipeModel.update((recipe) => ({
      ...recipe,
      ingredients: recipe.ingredients.map((ing, i) => (i === index ? { ...ing, name: name } : ing)),
    }));
  }
  ...
  addInstruction() {
    this.recipeModel.update((recipe) => {
      ...
      return {
        ...recipe,
        instructions: [...recipe.instructions, { index: newIndex, instruction: '' }],
      };
    });
  }
