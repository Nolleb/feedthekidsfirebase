...
export class RecipeService {
  ...
  recipesResource = rxResource<
    ...
    {
      pageSize?: number;
      lastDoc?: QueryDocumentSnapshot<RecipeDto> | null;
      direction?: 'next' | 'prev';
    }
  >({
  ...
  public createNewRecipeResource(recipe: RecipeDto): Observable<string> {
    ...
    return from(addDoc(recipesCollection, recipe)).pipe(map((docRef) => docRef.id));
  }
    ...
    return rxResource<RecipeDto | null, unknown>({
      ...
      stream: ({ params }) => {
        ...
        const recipeDoc = doc(
          ...
          'recipes',
          `${params}`,
        ) as DocumentReference<RecipeDto>;
        return docData<RecipeDto>(recipeDoc, { idField: 'id' }).pipe(map((data) => data ?? null));
      },
    });
  }

  public getRecipeResourceBySlug(
    categoryID: Signal<string | null>,
    config: Signal<RecipesListConfig>,
  ) {
    return rxResource<
      { recipes: RecipeDto[]; hasMore: boolean } | null,
      { id: string | null; config: RecipesListConfig }
    >({
      params: () => {
        const id = categoryID();
        const cfg = config();
  ...
  public getAdminRecipesResource(config: Signal<RecipesListConfig>) {
    return rxResource<{ recipes: RecipeDto[]; hasMore: boolean }, { config: RecipesListConfig }>({
      ...
      stream: ({ params }) => {
        ...
        const constraints: QueryConstraint[] = [orderBy('__name__'), limit(qLimit + 1)];
  ...
  public getLatestRecipesResource(count: number = 3) {
    return rxResource<RecipeDto[], unknown>({
      stream: () => {
        ...
        const q = query(recipesCollection, orderBy('createdAt', 'desc'), limit(count));
